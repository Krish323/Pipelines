###############################################################################
# Script: Add-AdUserInDatabasesBySqlUser.ps1
# Purpose: Add AD users to a given SQL Server databases using a Sql User.
# Parameters:
...
)
{
    ...
    $userIdentity = Get-AzADServicePrincipal -DisplayName $ActiveDirectoryUser -ErrorAction SilentlyContinue
...
Write-Host "`n`nExit Add-AdUserInDatabasesBySqlUser.ps1"